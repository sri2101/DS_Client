...
const api = (path) =>
  `${import.meta.env.VITE_API_BASE?.replace(/\/$/, "")}${path}`;
...
function Auth() {
  ...
  //     catch { console.error("Invalid JSON:", raw); }
  ...
  //       if (user?.type === 'ADMIN') console.log('login role: ADMIN');
  //       else if (user?.type === 'USER') console.log('login role: USER');
  //       else console.log('login role: unknown');

  //       const target = user?.type === 'ADMIN' ? '/admin/hotels' : '/';
  //       console.log('navigating to', target);
  //       navigate(target, { replace: true });
    ...
    if (res.ok) {
      ...
      if (user && Object.keys(user).length > 0) {
        ...
        try {
          ...
          if (profileRes.ok) {
            const profileJson = await profileRes.json();
            dispatch(setUser(profileJson.data));
          }
        } catch (e) {
      ...
      // fallback redirect for page state issues
      setTimeout(() => {
